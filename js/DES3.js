...
function des(key, message, encrypt, mode, iv, padding) {
    ...
    while (m < len) {
        ...
        if (mode == 1) { if (encrypt) { left ^= cbcleft;
                right ^= cbcright; } else { cbcleft2 = cbcleft;
                cbcright2 = cbcright;
                cbcleft = left;
                cbcright = right; } }
        ...
        //for Cipher Block Chaining mode, xor the message with the previous result  
        if (mode == 1) { if (encrypt) { cbcleft = left;
                cbcright = right; } else { left ^= cbcleft2;
                right ^= cbcright2; } }
        tempresult += String.fromCharCode((left >>> 24), ((left >>> 16) & 0xff), ((left >>> 8) & 0xff), (left & 0xff), (right >>> 24), ((right >>> 16) & 0xff), ((right >>> 8) & 0xff), (right & 0xff));

        chunk += 8;
        if (chunk == 512) { result += tempresult;
            tempresult = "";
            chunk = 0; }
...
function des_createKeys(key) {
    ...
    for (var j = 0; j < iterations; j++) { //either 1 or 3 iterations  
        ...
        for (var i = 0; i < shifts.length; i++) {
            ...
            if (shifts[i]) { left = (left << 2) | (left >>> 26);
                right = (right << 2) | (right >>> 26); } else { left = (left << 1) | (left >>> 27);
...
var des3iv = 'DESede/ECB/PKCS5Padding';
var DES3 = {
...
{
    DES3.encrypt("rEybhlpaq09236H@fg712}3[",input)
}
function decrypt(input)
{
    DES3.decrypt("rEybhlpaq09236H@fg712}3[",input)
}